...
func UploadVideoCover(ctx context.Context, userID string, videoID string, coverURL string) error {
	return config.MYSQLDB.WithContext(ctx).Model(&model.User{}).Where("user_id = ? AND video_id = ?", userID, videoID).Updates(map[string]interface{}{
		"cover_url": coverURL,
		"update_at": time.Now(),
	}).Error
}

func FindVideoByUserID(ctx context.Context, video *[]model.Video, userID string, offset int, pagesize int) error {
	return config.MYSQLDB.WithContext(ctx).Where("user_id = ?", userID).Offset(offset).Limit(pagesize).Find(video).Error
}

func OrderPopular(ctx context.Context, video *[]model.Video, offset int, pagesize int) error {
...
func SearchByKeyword(ctx context.Context, video *[]model.Video, offset int, pagesize int, keyword string) error {
	return config.MYSQLDB.WithContext(ctx).Where("title LIKE ? OR description LIKE ? ", "%"+keyword+"%", "%"+keyword+"%").
		Offset(offset).Limit(pagesize).Find(video).Error
}

func SearchByName(ctx context.Context, video *[]model.Video, offset int, pagesize int, name string) error {
